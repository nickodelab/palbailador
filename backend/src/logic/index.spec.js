...
describe('LOGIC', () => {
    ...
    describe('* USER * registerUser', () => {
        ...
        it('NOK - should thrown an error on non valid email', async () => {
            await expect(logic.registerUser({ ...user, email: 'failEmail@' })).to.be.rejectedWith(TypeError, 'non valid email')
        })

        it('NOK - should thrown an error on email = ""', async () => {
            await expect(logic.registerUser({ ...user, email: '' })).to.be.rejectedWith(TypeError, 'non valid email')
        })
        ...
        it('NOK - should thrown an error on nickname bellow min length', async () => {
            await expect(logic.registerUser({ ...user, nickname: 'hi' })).to.be.rejectedWith(Error, 'non valid nickname length')
        })

        it('NOK - should thrown an error on nickname over max length', async () => {
            await expect(logic.registerUser({ ...user, nickname: 'thisisanicknamereallyreallylong' })).to.be.rejectedWith(Error, 'non valid nickname length')
        })
        
        it('NOK - should thrown an error on nickname = ""', async () => {
            await expect(logic.registerUser({ ...user, nickname: '' })).to.be.rejectedWith(Error, 'non valid nickname length')
        })
        ...
        it('NOK - should thrown an error if nickname not contains chars alphanumeric', async () => {
            await expect(logic.registerUser({ ...user, nickname: 'this is my fail nickname' })).to.be.rejectedWith(Error, 'non valid nickname chars. Only letter and/or numbers')
        })

        it('NOK - should thrown an error on password bellow min length', async () => {
            await expect(logic.registerUser({ ...user, password: 'admin' })).to.be.rejectedWith(Error, 'password has to be 6 chars length minimum')
        })

        it('NOK - should thrown an error on passwor = ""', async () => {
            await expect(logic.registerUser({ ...user, password: '' })).to.be.rejectedWith(Error, 'password has to be 6 chars length minimum')
        })

        it('NOK - should throw an error if the user already exists', async () => {
            await User.create(user)
            await expect(logic.registerUser(user)).to.be.rejectedWith(Error, `user with email ${user.email} already exists`)            
        })
    })
    ...
    describe('* USER * authenticateUser', () => {
        ...
        it('NOK - should thrown an error on not found user', async () => {
            ...
            await expect(logic.authenticateUser('email@email.com', 'doesnt matters')).to.be.rejectedWith(Error, `user with email ${fakeEmail} not found`)
        })

        it('NOK - should thrown an error on wrong password', async () => {
            await expect(logic.authenticateUser(user.email, 'wrongpass')).to.be.rejectedWith(Error, 'wrong credentials')
        })

        it('NOK - should thrown an error on non valid email', async () => {
            await expect(logic.authenticateUser('failEmail@', '123456')).to.be.rejectedWith(TypeError, 'non valid email')
        })

        it('NOK - should thrown an error on email = ""', async () => {
            await expect(logic.authenticateUser('', '123456')).to.be.rejectedWith(TypeError, 'non valid email')
        })
        ...
        it('NOK - should thrown an error on password bellow min length', async () => {
            await expect(logic.authenticateUser('email@email.com', '12345')).to.be.rejectedWith(Error, 'password has to be 6 chars length minimum')
        })

        it('NOK - should thrown an error on password = ""', async () => {
            await expect(logic.authenticateUser('email@email.com', '')).to.be.rejectedWith(Error, 'password has to be 6 chars length minimum')
        })
    ...
    describe('* USER * createGroup', () => {
        ...
        it('OK - should succeed creating a group', async () => {
            try {
                const newGroup = await logic.createGroup(user.id, group.name, group.description)
                ...
                expect(searchNewGroup).to.have.property('description', group.description)
        ...
        it('NOK - should thrown an error on non exists user', async () => {
            await expect(logic.createGroup(fakeId, group.name, group.description)).to.be.rejectedWith(Error, 'user_not_found')
        })

        it('NOK - should thrown an error on userId = ""', async () => {
            await expect(logic.createGroup('', group.name, group.description)).to.be.rejectedWith(Error, 'non valid userId')
        })

        it('NOK - should thrown an error on userId = []', async () => {
            await expect(logic.createGroup([], group.name, group.description)).to.be.rejected
        })
        ...
        it('NOK - should thrown an error on name = ""', async () => {
            await expect(logic.createGroup(user.id, '', group.description)).to.be.rejectedWith(Error, 'non valid group name')
        })

        it('NOK - should thrown an error on name = []', async () => {
            await expect(logic.createGroup(user.id, [], group.description)).to.be.rejected
        })
        ...
        it('NOK - should thrown an error on description = ""', async () => {
            await expect(logic.createGroup(user.id, group.name, '')).to.be.rejectedWith(Error, 'non valid group description')
        })
    ...
    describe('* USER * deleteGroup', () => {
        ...
        beforeEach(async () => {
            ...
            const NewGroup = await Group.create({ ...group, members: [{ user: user.id, isAdmin: true }]})
            group.id = NewGroup._id.toJSON()
